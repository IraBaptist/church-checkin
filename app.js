...
function renderTeacher(){
  ...
  ${[...uniq.values()].map(r=>`<tr><td>${esc(r.studentName)}</td><td>${esc(r.grade)}</td><td>${r.recordId?`<button type="button" class="${r.present?"primary":""}" onclick="markPresent('${r.recordId}')">${r.present?"Present — click to undo":"Mark Present"}</button>`:"Not checked in"}</td></tr>`).join("")}
...
function weeklyMetrics(date){
  ...
  let teachers=0;
  const weekDays=dates().filter(x=>x>=start&&x<=e);
  weekDays.forEach(day=>{
    const att=state.staffAttendance[day]||{};
    Object.values(att).forEach(x=>{if(x&&x.status==="present")teachers++});
  });
...
$("parentForm").addEventListener("submit",async e=>{
  ...
  if(!email){$("parentMessage").textContent="Email is required."; $("parentMessage").classList.remove("hidden"); return}
  const phoneDigits=phone.replace(/\D/g,"");
  if(phoneDigits.length!==10){$("parentMessage").textContent="Please enter a valid 10-digit phone number."; $("parentMessage").classList.remove("hidden"); $("phone").focus(); return}
...
window.returnCheckin=async sid=>{
  ...
  const d=dates()[0];
  const existing=state.records.find(r=>r.studentId===s.studentId&&r.date===d);
  if(existing){
    $("parentMessage").textContent=`${s.studentName} is already checked in for this week.`;
    $("parentMessage").classList.remove("hidden");
    return;
  }
  const r={id:id(),studentId:s.studentId,studentName:s.studentName,parentName:s.parentName,email:s.email,phone:s.phone,age:s.age,grade:s.grade,group:s.group,date:d,checkedInAt:new Date().toISOString(),present:false};
